...
const {theme} = useTheme();
  ...
  useEffect(() => {
    console.log("Fetching payments for year:", yearfinal, "and userId:", user);
    ...
    const fetchPayments = async () => {
      ...
      try {
        ...
        console.log("we are here to year")
        ...
        console.log("we reached here for last")

        // 2️⃣ Get user yearly payment status
        const result = await payment(user.userId);

        console.log("result filal" ,result , yearfinal)


        const matchedYear = result.payment.find(
          (res) => Number(res.eth_year) === Number(yearfinal)
        );
